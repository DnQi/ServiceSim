...
public class CompK8SDevicesProvider1 extends DevicesProvider {

    int nodeNum;

    public CompK8SDevicesProvider1(int nodeNum) {
        super();
    ...
    public void createDevices(){
        ...
        ArrayList<NetworkDevice> smallBS_devices = createNetworkDevices("EdgeLevel1_DataCenter_", "edge", 1, 0, null,nodeNum, hostnum, pesList ,
                mips, ram, bw, storage);
        ArrayList<NetworkDevice> router = createNetworkDevices("Router_", "router", 2, nodeNum, null, 1, 1, pesList , mips, ram, bw, storage);

        for (int i = 0;i<smallBS_devices.size();i++){

            if (i==3){
                Channel channel = new Channel(smallBS_devices.get(i).getId(),router.get(0).getId(),15 * 1024 * 1024,0.0001,new PacketSchedulerTimeShared());
                ...
                Channel channel1 = new Channel(router.get(0).getId(),smallBS_devices.get(i).getId(),15 * 1024 * 1024,0.0001,new PacketSchedulerTimeShared());
                ...
                Channel channel = new Channel(smallBS_devices.get(i).getId(),router.get(0).getId(),70 * 1024 * 1024,0.0001,new PacketSchedulerTimeShared());
                smallBS_devices.get(i).getParentDeviceIds().add(router.get(0).getId());
                smallBS_devices.get(i).getParentDevicesToChannel().put(router.get(0).getId(),channel);

                Channel channel1 = new Channel(router.get(0).getId(),smallBS_devices.get(i).getId(),70 * 1024 * 1024,0.0001,new PacketSchedulerTimeShared());
                router.get(0).getChildDeviceIds().add(smallBS_devices.get(i).getId());
                router.get(0).getChildDevicesToChannel().put(smallBS_devices.get(i).getId(),channel1);
            }
            else{
